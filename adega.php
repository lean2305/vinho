...
<link rel="stylesheet" href="css/adega.css">
      ...
      <div class="chart-container-wrapper big">
        <div class="chart-containerr">
          <div class="chart-container-header">
            <h2>Alterações</h2>
            <span>Ultimas 20 alterações</span>
          </div>
...
<table class="fixed_headers">
  ...
  <tbody>
    <?php $sql ="SELECT * FROM historico  ORDER BY `historico`.`id_historico` DESC LIMIT 20";
...
if($res=mysqli_query($conn,$sql)){
    ...
    $id[$iol] = $reg['id_historico'];
    $funcionario[$iol] = $reg['funcionario'];
    $descricao[$iol] = $reg['descricao'];
 ?>
    <tr>
      <td><?php echo $id[$iol]; ?></td>
      <td><?php echo $funcionario[$iol]; ?></td>
      <td><?php echo $descricao[$iol]; ?></td>
    </tr>
...
<button onclick="parent.location='adegaall.php'" class="select"  type="submit" class="bubbly-button">Ver tudo</button> 
...
if($res=mysqli_query($conn,$sql)){
    ...
    $min[$iol] = $reg['quant_min'];
    $max[$iol] = $reg['quant_max'];
    ...
    if($disp[$iol] < $min[$iol]){ 
      ...
      $armario[$iol] = $reg['nome_armario'];
      ?>
              <span><?php echo $armario[$iol]; ?></span>
              <p>Atual:<?php echo $disp[$iol]; ?> <strong>Min:<?php echo $min[$iol]; ?></strong> Max:<?php echo $max[$iol]; ?> </p>
      <?php
    }
    else{
